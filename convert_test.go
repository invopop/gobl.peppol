...
// contextPINT mirrors the Peppol PINT A-NZ billing context that the conversion
// layer of this module will provide.
...
// UBL document and compares it against its golden XML. Run with -update to
...
func TestConvert(t *testing.T) {
	for _, example := range convertCases(t) {
		t.Run(example.name, func(t *testing.T) {
			...
			doc, err := ubl.ConvertInvoice(env, ubl.WithContext(contextPINT))
			require.NoError(t, err)
